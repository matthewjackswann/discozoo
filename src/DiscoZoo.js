...
const DiscoZoo = () => {
    ...
    return <div className="discoZoo">
        <p>
            ...
            For an explanation on how it works and the full code goto // todo add stuff here<br />
        </p><br />
                ...
                return <div key={y} className="discoRow">
                    ...
                    return <div key={x} onClick={(e) => {getPossibleTiles(e, x, y)}} style={{filter: "brightness(" + (squareKnown ? "1" : "0.5") + ")"}} className="discoSquare">
                        {Object.keys(square).sort((a, b) => square[b] - square[a] || a.localeCompare(b)).map(animal => {
                            return <img key={animal} style={{width: 100 * square[animal] / gridNumber + "%"}} alt={animal} src={require("./imgs/" + animal + ".png")} />
                        })}
                    ...
                    {dropDown === i && <div>
                        {animalInfo.area[area].map((animal, key) => {
                            return <div className="discoAnimalC" key={key}>
                                <img alt={animal} src={require("./imgs/" + animal + ".png")} onClick={() => {addAnimal(animal)}} style={{cursor: Object.getOwnPropertyNames(animals).includes(animal) ? "" : "pointer"}} />
